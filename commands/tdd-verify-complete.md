...
- `docs/implementation/{test_case_name}-memo.md` を確認（メモファイルが存在する場合）
...
#### メモファイルへの追記
検証完了後、以下の情報を `docs/implementation/{test_case_name}-memo.md` に追記：
...
- **検証フェーズ**: verify-complete
- **既存テスト状態**: [グリーン/失敗テストあり]
...
- `docs/implementation/{feature_name}-requirements.md`
- `docs/implementation/{feature_name}-testcases.md`
...
- テストケース実装率: 100%未満
- 重要な未実装テストが存在
- 要件網羅率: 80%未満（不十分）または重要な要件項目が未実装
- 品質リスクあり
- 追加実装の判断が必要
```
